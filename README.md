...
Para clonar e executar esta aplicação no seu ambiente local, siga os passos detalhados abaixo. É crucial que os comandos sejam executados na ordem correta.

1.  **Clone o repositório:**
    Primeiro, clone o projeto do GitHub para a sua máquina local usando o comando:
    ```bash
    git clone https://github.com/davihoracio/Projeto-Pagina-de-Login---Desafio-B2Bit
    ```

2.  **Acesse a pasta do projeto (Passo Essencial!)**
    Após clonar, o Git cria uma nova pasta. Você **precisa** entrar nela antes de prosseguir:
    ```bash
    cd Projeto-Pagina-de-Login---Desafio-B2Bit
    ```

3.  **Instale as dependências**
    Agora, dentro da pasta do projeto, execute o comando abaixo para instalar todas as bibliotecas necessárias (React, Axios, etc.):
    ```bash
    npm install
    ```

4.  **Inicie a aplicação**
    Finalmente, inicie o servidor de desenvolvimento local:
    ```bash
    npm run dev
    ```

A aplicação estará disponível para acesso no seu navegador, geralmente no endereço `http://localhost:5173`.
